...
def fit_umap(
    ...
    output_columns=["umap_x", "umap_y"],
    target_y: str = None,
    embeddings_in_list=False,
    **kwargs,
):
    ...
    target_y : str, optional
        Name of a column to use as the target variable for supervised UMAP,
        by default None
    ...
    **kwargs
        Additional arguments to pass to UMAP. Most important is n_neighbors
        (default is 15).
    ...
    if target_y:
        if target_y not in df.columns:
            raise KeyError(
                f"Your target_y value {target_y} should be the name of"
    ...
    if embeddings_in_list:
        if len(input_columns) > 1:
            raise ValueError(
                "If your embeddings are in a list, they should be in a single column."
            )
    ...
    adjusted_n_neighbors = min(
        original_n_neighbors, max(2, embedding_data.shape[0] - 1)
    )

    if adjusted_n_neighbors != original_n_neighbors:
        warnings.warn(
            f"n_neighbors adjusted from {original_n_neighbors} to"
            f" {adjusted_n_neighbors} due to small dataset size."
        )
...
@pf.register_dataframe_method
def fit_cluster_hdbscan(
    df,
    input_columns=["umap_x", "umap_y"],
    output_columns=["cluster", "cluster_probability"],
    min_cluster_size=5,
    min_samples=None,
    ...
    -------
    pandas.DataFrame
        The input DataFrame with additional columns containing cluster labels
        and probabilities.
    """
